...
public class NGC3R3 : MonoBehaviour
{
    ...
    private IEnumerator Rotate()
    {
        ...
        for (int i = 0; i < 30; i++)
        {
            if (transform.position.y <= -16f)
                transform.position = new Vector2(transform.position.x, 7.25f);
        ...
        randomValue = Random.Range(110, 120);
        ...
        if (transform.position.y == 5.75f)
            ...
            stoppedSlot = "25";
        else if (transform.position.y == -11.50f)
            stoppedSlot = "26";
