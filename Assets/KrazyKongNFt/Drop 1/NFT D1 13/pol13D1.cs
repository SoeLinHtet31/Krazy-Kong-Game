...
public class pol13D1 : MonoBehaviour
{
    async void Start()
    {
        ...
        string tokenId = "168341381845511608478122282122273576593983094670";

        BigInteger balanceOf = await ERC1155.BalanceOf(chain, network, contract, account, tokenId);
        print(balanceOf);

        if (balanceOf > 0 && NFTD1Main13.NFt13D1Amount == 0)
        {
            ...
            NFTD1Main13.NFt13D1Amount += 1;
            PlayerPrefs.SetInt("NFt13D1Amount", NFTD1Main13.NFt13D1Amount);

            SKYBITToken.SkyBitTokengainAmount += 100;
            PlayerPrefs.SetInt("skyBitTokengainAmount", SKYBITToken.SkyBitTokengainAmount);
        }
        else if (balanceOf <= 0 && NFTD1Main13.NFt13D1Amount == 1)
        {
            AmountOFNFTHOLD.amountOfFirstKrazyKongNFt -= 1;
            PlayerPrefs.SetInt("amountOfFirstKrazyKongNFt", AmountOFNFTHOLD.amountOfFirstKrazyKongNFt);
