...
public class pol53 : MonoBehaviour
{
    async void Start()
    {
        ...
        string tokenId = "0x720621590290a0856e5e6a4abfbee5330ba95f3d";

        BigInteger balanceOf = await ERC1155.BalanceOf(chain, network, contract, account, tokenId);
        print(balanceOf);

        if (balanceOf > 0 && NFT53Main.NFt53Amount == 0)
        {
            ...
            NFT53Main.NFt53Amount += 1;
            PlayerPrefs.SetInt("NFt53Amount", NFT53Main.NFt53Amount);

            SKYBITToken.SkyBitTokengainAmount += 20;
            PlayerPrefs.SetInt("skyBitTokengainAmount", SKYBITToken.SkyBitTokengainAmount);
        }
        else if (balanceOf <= 0 && NFT53Main.NFt53Amount == 1)
        {
            AmountOFNFTHOLD.amountOfFirstKrazyKongNFt -= 1;
            PlayerPrefs.SetInt("amountOfFirstKrazyKongNFt", AmountOFNFTHOLD.amountOfFirstKrazyKongNFt);
