...
public class pol79 : MonoBehaviour
{
    async void Start()
    {
        ...
        string tokenId = "0xd7232e44e527c018537f237297f446c6a19dec0a";

        BigInteger balanceOf = await ERC1155.BalanceOf(chain, network, contract, account, tokenId);
        print(balanceOf);

        if (balanceOf > 0 && NFT79Main.NFt79Amount == 0)
        {
            ...
            NFT79Main.NFt79Amount += 1;
            PlayerPrefs.SetInt("NFt79Amount", NFT79Main.NFt79Amount);

            SKYBITToken.SkyBitTokengainAmount += 40;
            PlayerPrefs.SetInt("skyBitTokengainAmount", SKYBITToken.SkyBitTokengainAmount);
        }
        else if (balanceOf <= 0 && NFT79Main.NFt79Amount == 1)
        {
            AmountOFNFTHOLD.amountOfFirstKrazyKongNFt -= 1;
            PlayerPrefs.SetInt("amountOfFirstKrazyKongNFt", AmountOFNFTHOLD.amountOfFirstKrazyKongNFt);
